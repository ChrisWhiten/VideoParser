...
void ObjectRecognizer::clusterShapes()
{
	...
	for (auto c = all_classes.begin(); c != all_classes.end(); ++c)
	{
		...
		{
			KMedoids kmedoids;
			kmedoids.init(shape_context_spgs, m_params.cluster_k, m_pShapeMatcher);
			kmedoids.run();
			clusterings.push_back(kmedoids);

			if (kmedoids.current_cost < best_cluster_score)
			{
				best_cluster = cluster;
				best_cluster_score = kmedoids.current_cost;
			}
		}
		...
		{
			// the inner dereferencing is to an element of clusters, which is an index into points,
			// which points to a KMedoidsPoint, which has an id element pointing to the actual id.
			...
			if (model_hierarchy.GetModelObject(model_view.viewInfo.storParentMetadataId).classInfo.id < 0)
			{
				...
				cout << model_hierarchy.GetModelObject(model_view.viewInfo.storParentMetadataId).classInfo.name << endl;
				cout << model_hierarchy.GetModelObject(model_view.viewInfo.storParentMetadataId).classInfo.id << endl;
				cout << model_hierarchy.GetModelObject(model_view.viewInfo.storParentMetadataId).classInfo.storDataId << endl;
	...
	file.close();
	cout << "Clustering completed.  Exiting." << endl;
	exit(0);
}
